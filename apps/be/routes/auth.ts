...
import { generateMagicLinkToken, generateSessionToken, hashToken } from '@/packages/auth';
...
export async function handleRequestMagicLink(req: Request): Promise<Response> {
  ...
  return Response.json({ message: 'If that email is registered, a magic link has been sent.' });
}
...
export async function handleVerifyMagicLink(req: Request): Promise<Response> {
  ...
  const magic = await prisma.magicLinkToken.findUnique({ where: { tokenHash: hash } });

  if (!magic)              return Response.json({ error: 'Invalid token' },  { status: 401 });
  if (magic.usedAt)        return Response.json({ error: 'Token already used' }, { status: 401 });
  if (magic.expiresAt < new Date()) return Response.json({ error: 'Token expired' }, { status: 401 });
...
export async function requireAdmin(req: Request): Promise<{ user: any } | Response> {
  const user = await getUserFromRequest(req);
  if (!user) return Response.json({ error: 'Unauthorized' }, { status: 401 });
